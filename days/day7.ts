...
function parseInput(input: string[]): IProgram[] {
  return input.map((str) => {
    const matches = treeReg.exec(str);
    if (matches) {
      const name = matches[1];
      const weight = parseInt(matches[2], 10);
      let children: string | string[] = matches[3] || [];
...
function findChildProgram(tree: IProgram[], childName: string) {
  return tree.find((p) => p.name === childName);
}

function sortTree(tree: IProgram[]) {
  const sortedInput: IProgram[] = [];

  const sort = (program: IProgram) => {
    if (program.children.length) {
      const subPrograms: IProgram[] = [];
      program.children.forEach((child) => {
        const subprogram = findChildProgram(tree, child);
        if (subprogram) {
          sortedInput.push(subprogram);
          subPrograms.push(subprogram);
        } else {
          throw new Error('Invalid input');
        }
      });
      subPrograms.forEach((sp) => sort(sp));
    } else {
      sortedInput.push(program);
    }
    return sortedInput;
  };

  const rootTree = root(tree);
  if (rootTree) {
    return [rootTree, ...sort(rootTree)];
  }
  throw new Error('No root ?');
}

function root(tree: IProgram[]) {
  return tree.find((program) => tree.every((subprogram) => !subprogram.children.includes(program.name)));
}

export function findRootProgram(input: string[]) {
  const rootTree = root(parseInput(input));
  if (rootTree) {
...
export function findWeight(input: string[]) {
  const tree = sortTree(parseInput(input)).reverse();
  let i = 0;
  ...
  while (i < tree.length) {
    ...
    const children = tree[i].children
      .reduce((map, child) => {
        const childProgram = findChildProgram(tree, child);
        if (childProgram) {
          const totalWeight = childProgram.children.reduce((p, n) => {
            const subChildProgram = findChildProgram(tree, n);
            if (!subChildProgram) {
              throw new Error(`Cannot find ${tree[i].name} child ${child} subchild ${n}`);
            }
            p += subChildProgram.weight;
            return p;
          }, childProgram.weight);

          if (map.has(totalWeight)) {
            (map.get(totalWeight) as IProgram[]).push(childProgram);
          } else {
            map.set(totalWeight, [childProgram]);
          }
          return map;
        }
        throw new Error(`Cannot find ${tree[i].name} child ${child}`);
      }, new Map<number, IProgram[]>());

    if (children.size !== 2) {
      if (children.size !== 1) {
        console.warn(i, tree[i].name, tree[i].children.length, children.size);
      }
      i++;
      continue;
    }

    let normalWeight: number | undefined;
    let program: IProgram | undefined;
    let programWeight: number | undefined;
    children.forEach((v, k) => {
      if (v.length > 1) {
        ...
        throw new Error(`Overweighted program should be isolated`);
      }
    });
    if (!normalWeight || !programWeight || !program) {
      throw new Error('Overweighted program should be isolated');
    }

    expectedWeight = program.weight - (programWeight - normalWeight);
    break;
  }
