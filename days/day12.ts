...
function connectedPrograms(programs: IProgram[], connections: number[]) {
  ...
  while (length < connections.length) {
    ...
    programs.forEach((program) => {
      if (program.connections.some((c) => connections.includes(c))) {
        connections.push(...[program.id, ...program.connections]
          .filter((pid) => !connections.includes(pid)));
      }
    });
  }
...
export function connectionsCount(input: string[]) {
  return connectedPrograms(parseInput(input), [0]).length;
}

export function connectionsGroups(input: string[]) {
