...
const operationsReg = /([a-z]*)\s([a-z]*)\s(-?\d*)\sif\s([a-z]*)\s([^\s]*)\s(-?\d*)/;

interface IOperationCondition {
  operator: string;
  value: number;
  variable: string;
}
...
interface IOperation {
  condition: IOperationCondition;
  operator: string;
  offset: number;
  variable: string;
}

function parseOperations(input: string[]): IOperation[] {
  return input.map((op) => {
...
function initializeMap(operations: IOperation[]) {
...
function isConditionFullfilled(condition: IOperationCondition, map: Map<string, number>) {
...
function doOperation(operation: IOperation, map: Map<string, number>) {
...
function doOperations(operations: IOperation[], map: Map<string, number>) {
...
function findMapLargest(map: Map<string, number>, largest = Number.NEGATIVE_INFINITY) {
...
export function findLargest(input: string[]) {
...
export function findLargestEver(input: string[]) {
  const operations = parseOperations(input);
  const mapOfValues = initializeMap(operations);
